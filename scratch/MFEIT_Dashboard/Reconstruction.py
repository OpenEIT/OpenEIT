...
class Reconstruction(mp.Process):
	...
	def store_frequency_data(self, freq, img): # How is this stored? 
		# 
		if freq: 
			self.d[int(float(freq.replace(" ","")))] = img 
		# print (self.d.keys())

	def get_frequency_data(self, pixel_x,pixel_y):
		# print(pixel_x,pixel_y)
		# Check that x and y are <100 and integer:
		if pixel_x < 100 and pixel_y <100 and len(self.d) >= 10:
			spect 				= []
			sorted_frequencies 	= []
			it 					= 0
			# return the spectrogram at a given pixel. 
			for key in sorted(self.d):
				im = self.d[key]
				spect.append(im[pixel_x,pixel_y])
				sorted_frequencies.append(key)
				it = it+1
			# 
			# print (sorted_frequencies)
			# print (spect)
		else: 
			spect = [0] * len(self.d) #17 
		return spect

	def get_freq_dict(self):
		return self.d 

	def get_freq_values(self): # return the actual frequency values. 
		freqs = []
		for key in sorted(self.d):
			freqs.append(key)
		return freqs
